...
if (_terrain in KTWK_BML_excluded) exitWith {
    KTWK_BML_set = true;
    ["KTWK_BML_set", true, true] remoteExec ["setVariable", _targets];
    false
};
...
private _aperture = call {
    if !(_terrain in KTWK_BML_noAperture) exitWith {
        if (_terrain in KTWK_BML_altAperture_narrow) exitWith {
            [4.5, 4.6, 4.7, 0.9]
        };
        if (_terrain in KTWK_BML_altAperture_mid) exitWith {
            [3.5, 3.6, 3.7, 0.9]
        };
        if (_terrain in KTWK_BML_altAperture_wide) exitWith {
            [2.7, 2.8, 2.9, 0.9]
        };
        if (_terrain in KTWK_BML_altAperture_ultraWide) exitWith {
            [2.3, 2.4, 2.5, 0.9]
        };
        [4, 4.1, 4.2, 0.9]
    };
...
private _debugStr = format ["[Brighter Moonlight] %1 night applied", ["", "Bright", "Brighter"] select KTWK_BML_opt_enabled];

call {
    if (_targets isEqualTo []) exitWith {
        KTWK_BML_set = true;
        [_effect, _aperture, _noWait] spawn KTWK_BML_fnc_set_client;
        if (!isNil "KTWK_opt_debug" && {KTWK_opt_debug}) then {
            systemChat _debugStr;
        };
    };
    [_effect, _aperture, _noWait] remoteExec ["KTWK_BML_fnc_set_client", _targets];
    if (!isNil "KTWK_opt_debug" && {KTWK_opt_debug}) then {
