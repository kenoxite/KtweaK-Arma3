...
if (time - (_group getVariable ["KTWK_lastCombatCheck", 90]) > _checkTimer) then {
    ...
    if (!isNull _nearEnemy && {_unit distance _nearEnemy < 100}) then {
        _group setVariable ["KTWK_lastCombatTime", time, false];
        _return = true;
    } else {
        if (time - (_group getVariable ["KTWK_lastCombatTime", 0]) > _outOfCombatTime) then {
            _return = false;
        };
    };
    _group setVariable ["KTWK_inCombat", _return, false];
    _group setVariable ["KTWK_lastCombatCheck", time, false];
};
